...
import styles from './SociotekaSocial.module.scss';
import { ChangeEvent, useEffect, useState } from 'react';
import { useQueryParams } from 'hooks';
import { useSocials } from 'hooks/queries/entities/useSocials';
import { SocialsFiltration } from '../SocialsFiltration/SocialsFiltration';

export const SociotekaSocial = () => {
  ...
  const { socials, isLoading, getSocials } = useSocials();

  useEffect(() => {
    getSocials(page, limit);
  }, [page, limit]);
  ...
  const handleColumnHeaderClick = (columnHeader: string) => {
    ...
    if (page === 1) {
      getSocials(page, limit, preparedQueryParams as any);
    } else {
      setPage(1);
    }
  };
    ...
    <div className={styles.wrapper}>
      <Wrapper>
        <div className={styles.content}>
          ...
          <EntitiesNavigation className={styles.navigation} active={4} />
          <Search
